...
class App extends Component {
  ...
  getChess = () => {
        ...
        for (var j = 0; j < i + 1; j++) {
          position.push({
            ...
            style: {
              background: colors[k],
              left: Math.round(
                Math.cos((k * 60 * 2 * Math.PI) / 360) *
                  (width -
                    ((width * 2) / (4 * 3 * 2)) * i +
                    (j * width * 2) / (4 * 3) -
                    width) -
                  Math.sin((k * 60 * 2 * Math.PI) / 360) *
                    (((i * gap + 2 * radius * i) * Math.sqrt(3)) / 2 -
                      rotateY) +
                  width
              ),
              top: Math.round(
    ...
    //Get the top half of the 6-sided polygon
    for (var i = 0; i < 5; i++) {
      for (var j = 0; j < i + 5; j++) {
        position.push({
          key: `hex-up-${i}-${j}`,
          ...
          style: {
            ...
            top: Math.round(
              rotateY / 2 + ((((width * 2) / (4 * 3)) * Math.sqrt(3)) / 2) * i
            ),
            zIndex: 1,
          },
        });
      ...
      for (var j = 0; j < 13 - i; j++) {
        position.push({
          ...
          style: {
            ...
            top: Math.round(
              (rotateY * 3) / 2 -
                ((((width * 2) / (4 * 3)) * Math.sqrt(3)) / 2) * 3 +
                ((((width * 2) / (4 * 3)) * Math.sqrt(3)) / 2) * (i - 5)
            ),
            zIndex: 1,
          },
        });
  ...
  chessMove = (key) => (e) => {
    ...
    if (press && permission.length > 0) {
      ...
      let styles = this.getAnimatePathStyle(
        this.defineAnimatePath(destination, start, end).reverse(),
        destination
      );
  ...
  getParents = (locate, path) => {
    ...
    if (parents.length < 1) {
      console.log(`${locate}Parent`, parents);
    }
  ...
  defineAnimatePath = (path, start, end) => {
    console.log("Don’t count the checkers animation");
    console.log("path:", path, "Starting Point", start, "Ending Point", end);
  ...
  resetChessColor = () => {
    ...
    const style = Object.assign({}, lastPress.style, {
  ...
  getValidPoint = (itemMove, position, allPath, passNode) => {
    ...
    if (this.exist(itemMove, passNode).length <= 1) {
      ...
      arrs = arrs.map((arr) => {
        ...
        arr = arr.sort((arg1, arg2) => {
          return arg1.locate.split("-").slice(0, 2).join("") * 1 >
            arg2.locate.split("-").slice(0, 2).join("") * 1
            ? 1
            : -1;
        });
        return arr;
      ...
      arrs.map((arr, index) => {
        ...
        for (; left >= 0; ) {
          if (arr[left].isOccupy) {
            console.log("Base Point", arr[left], "Starting Point", itemMove);
            let flag = false;
            let end = left * 2 - pos;
            if (end < 0 || end >= arr.length) {
              console.log(
                "The symmetry point exceeds the Checker",
        ...
        for (; right < arr.length; ) {
          if (arr[right].isOccupy) {
            console.log("", arr[right], "", itemMove);
            let flag = false;
            let end = right * 2 - pos;
            if (end < 0 || end >= arr.length) {
            ...
            for (
              let start = right + (end - right) / Math.abs(end - right);
    ...
    return nextJump
      ? this.getValidPoint(nextJump, position, allPath, passNode)
      : passNode;
  };
      ...
      path.map((pathNode) => {
        ...
        if (tempoPath.length === 0) {
          ...
          for (let i = 0; i < tempoPath.length; i++) {
            firstPathNode["parent"] = firstPathNode["parent"].concat(
              tempoPath[i]["parent"]
            );
          }
